...
export default function ConnectHubNara() {
  ...
  const handleStart = (opt) => {
    ...
    const firstMsg = {
      role: 'assistant',
      content: `Seja muito bem-vindo(a)! Escolher o caminho "${opt.label}" é um passo importante. Sou a NARA, sua mentora aqui no ecossistema. Estou pronta para te ouvir com toda atenção. Conte para mim: o que trouxe você até aqui hoje?`
    };
  ...
  return (
    <main className="flex flex-col min-h-screen bg-[#0a192f] text-slate-100 font-sans selection:bg-green-500/30">
      <header className="p-5 border-b border-white/10 bg-[#0a192f]/80 backdrop-blur-md sticky top-0 z-50 flex justify-between items-center">
        ...
        <div className="px-3 py-1 rounded-full border border-green-500/30 bg-green-500/10 text-[10px] text-green-400 font-bold uppercase tracking-tighter">NARA • Maestrina v1.0</div>
      </header>
      ...
      {step === 'welcome' ? (
        ...
        <section className="flex-1 flex flex-col max-w-4xl mx-auto w-full p-4 overflow-hidden animate-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="p-2 bg-white/5 rounded-3xl border border-white/10 mt-4 flex items-center gap-2 focus-within:border-green-400/50 transition-colors shadow-2xl">
            <input className="flex-1 bg-transparent outline-none text-sm px-4 py-3 placeholder:text-white/20 text-white" placeholder="Fale com a NARA..." value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendMessage()} />
            <button onClick={sendMessage} disabled={loading || !input.trim()} className="p-3 bg-green-500 rounded-2xl hover:bg-green-400 disabled:opacity-50 transition-all text-black shadow-lg shadow-green-500/20"><Send size={18} /></button>
          </div>
        </section>
      )}
    </main>
